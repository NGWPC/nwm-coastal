...
# ## Setup
#
# Change to the `narragansett-ri/` directory so that relative paths in
...
# datasets, mask thresholds, subgrid parameters, and NWM discharge
# configuration.
...
#
# This downloads elevation data (NOAA NWS 30 m topobathy + GEBCO
# bathymetry) and ESA WorldCover land-use, then builds the quadtree
# grid, elevation, mask, boundary conditions, discharge source points,
# and subgrid tables.
...
# download paths, and SFINCS runtime parameters.  Note the
# `discharge_locations_file` field which points to the `.src` file
# created by the model build step — this enables NWM streamflow forcing.
...
# bay are at the base resolution.  The map output file (`sfincs_map.nc`)
# stores the mesh as standard UGRID quadrilaterals.
...
ax.set_title("Narragansett Bay SFINCS quadtree mesh")
plt.show()
...
ax.set_title("Narragansett Bay flood depth (hmax) from SFINCS simulation")
plt.show()
...
# via the CLI:
#
# 1. `coastal-calibration create create.yaml` — built the model from an
#    AOI with NWM discharge source points
# 2. `coastal-calibration run run.yaml` — downloaded data, ran SFINCS
#    with compound forcing (ocean + river + meteo), and compared results
