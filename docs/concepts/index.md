...
from a single configuration file either through command-line interface or Python API.
Routed NWM streamflow enters at the models' river inflow points, tides and storm surge 
at the ocean boundary, and meteorological data over the domain surface. From these 
inputs, the models simulate total water level and coastal flooding. NOAA CO-OPS water 
level observations can be downloaded automatically and compared against the simulated 
results.

Each NWM v3.0 coastal domain has an existing SCHISM mesh, and this package can subset
one to a smaller region of interest. Automated SCHISM mesh generation has not been
implemented into this package and is a possible target for future development. SFINCS 
is included for a complementary capability: HydroMT-SFINCS can build a complete model
 — grid, elevation, roughness, and boundaries — programmatically from an 
 area-of-interest polygon, making it practical to stand up coverage or improve 
 resolution wherever it may be needed.
