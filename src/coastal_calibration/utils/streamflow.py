...
def read_streamflow(
    ...
    *,
    ...
    if chrtout_files is None:
        raise ValueError(
            "chrtout_files is required when meteo_source is 'nwm_ana'"
        )
...
def _read_from_zarr(
    ...
    *,
    ...
    if url is None:
        raise ValueError(
            f"No Zarr store configured for domain {domain!r}. "
            f"Available: {sorted(_ZARR_STORES)}"
        )
    ...
    if not keep:
        logger.warning(
            "None of the requested feature_ids found in Zarr store"
        )
        return pd.DataFrame()

    sf = (
        ds["streamflow"]
        .sel(feature_id=keep, time=slice(start, end))
        .load()
    )
...
def _read_from_chrtout(
    ...
    if not keep_idx.size:
        logger.warning(
            "None of the requested feature_ids found in CHRTOUT files"
        )
        return pd.DataFrame()
