...
def patch_serialize_crs() -> None:
    ...
    _log.debug("Patched hydromt _serialize_crs to handle CRS without an authority.")
...
def patch_boundary_conditions_index_dim() -> None:
    ...
    _log.debug("Patched hydromt-sfincs _validate_and_prepare_gdf to normalize index name.")
...
def patch_meteo_write_gridded() -> None:
    ...
    _log.debug("Patched hydromt-sfincs write_gridded to stream time-steps lazily.")
...
def patch_quadtree_subgrid_data_setter() -> None:
    ...
    _log.debug("Patched SfincsQuadtreeSubgridTable.data to add missing setter.")
...
def patch_quadtree_output_read() -> None:
    ...
    _log.debug("Patched SfincsOutput.read_map_file to re-grid quadtree map output to UGRID.")
