...
class SfincsInitStage(_SfincsStageBase):
    ...
    #: Files that are regenerated by downstream stages and must be
    #: restored from the pre-built model on every re-run.  Without
    #: this, a stale ``sfincs.obs`` from a previous buggy run that
    #: contained duplicate entries would never be overwritten because
    #: the default copy logic skips files that already exist.
    _ALWAYS_OVERWRITE_FILES: tuple[str, ...] = ("sfincs.obs",)
...
class SfincsDischargeStage(_SfincsStageBase):
    ...
    def _filter_active_cells(
        ...
        if model.grid_type == "quadtree":
            ...
            pts_x, pts_y = zip(*points, strict=False)
...
class SfincsPlotStage(_SfincsStageBase):
    ...
    * No observation points with the ``noaa_`` prefix are found.
    ...
    @staticmethod
    def _match_noaa_stations(
        mod: Any,
        match_distance_m: float = 100.0,
    ) -> tuple[list[int], list[str]]:
        """Match observation points to NOAA CO-OPS stations by proximity.

        For each observation point in the model, the closest CO-OPS
        station within *match_distance_m* metres (in the projected model
        CRS) is recorded.  Each CO-OPS station is matched at most once.

        Parameters
        ----------
        mod : SfincsModel
            An opened SFINCS model with a CRS and observation points.
        match_distance_m : float, optional
            Maximum distance (metres) for a match.  Default 100 m.

        Returns
        -------
        noaa_indices : list[int]
            Observation-point indices (into ``mod.observation_points.data``)
            that matched a CO-OPS station.
        noaa_station_ids : list[str]
            Corresponding CO-OPS station IDs.
        """
        import contextlib
        ...
        client = COOPSAPIClient()
        stations_gdf = client.stations_metadata
        region_4326 = mod.region.to_crs(4326)
        domain_stations = stations_gdf[stations_gdf.within(region_4326.union_all())]

        obs_gdf = None
        with contextlib.suppress(Exception):
            obs_gdf = mod.observation_points.data
        if obs_gdf is None or obs_gdf.empty or domain_stations.empty:
            return [], []

        model_crs = mod.crs
        if model_crs is not None:
            domain_stations = domain_stations.to_crs(model_crs)

        noaa_indices: list[int] = []
        noaa_station_ids: list[str] = []
        seen_ids: set[str] = set()
        for obs_idx in range(len(obs_gdf)):
            geom = obs_gdf.geometry.iloc[obs_idx]
            if geom is None:
                continue
            ox, oy = geom.x, geom.y
            for _, st_row in domain_stations.iterrows():
                sid = st_row["station_id"]
                if sid in seen_ids:
                    continue
                if math.hypot(ox - st_row.geometry.x, oy - st_row.geometry.y) < match_distance_m:
                    noaa_indices.append(obs_idx)
                    noaa_station_ids.append(sid)
                    seen_ids.add(sid)
                    break
    ...
    def run(self) -> dict[str, Any]:
        ...
        # Identify NOAA stations by spatial proximity
        noaa_indices, noaa_station_ids = self._match_noaa_stations(mod)
