...
class CoastalPreprocessorApp:
    @staticmethod
    def _build_grid_(
        latitudes: np.ndarray,
        longitudes: np.ndarray,
        corner_lats: np.ndarray = None,
        corner_lons: np.ndarray = None,
    ):
        ...
        stagger = (
            ...
            else [ESMF.StaggerLoc.CENTER]
        )
        # noinspection PyTypeChecker
        grid = ESMF.Grid(
            np.array([nlon, nlat]), staggerloc=stagger, coord_sys=ESMF.CoordSys.SPH_DEG
        )
    ...
    def _setup_latlon_grid_(self, ds: Dataset):
        ds.variables["XLAT_V"]
        ds.variables["XLONG_U"]
        ds.variables["XLAT_M"]
        ds.variables["XLONG_M"]
        ...
        dlat = dlon = 0.01
        self.lats = np.arange(
            math.floor(lat_min / dlat) * dlat, (math.ceil(lat_max / dlat) * dlat) + dlat, dlat
        )
        self.lons = np.arange(
            math.floor(lon_min / dlon) * dlon, (math.ceil(lon_max / dlon) * dlon) + dlon, dlon
        )
    ...
    def regrid_all_files(
        self, output_path_transformer, var_filter=None, file_filter=None, skip_latlon=False
    ):
        ...
        if self.root:
            ...
            if idx == 0:
                self.schism_vsource = Dataset(
                    Path(self.output_dir / "precip_source.nc"), "w", format="NETCDF4"
                )
                self._build_source_nc(
                    self.schism_vsource,
                    ntimes=len(input_files),
                    elems=np.arange(1, self.total_elements + 1),
                )
        ...
        for file in input_files:
            if not skip_latlon:
                if file in sub_input_files:  # only process our task's subset
                    if self.root:
                        pass
                    self.regrid_to_lat_lon(
                        file, output_path_transformer=output_path_transformer, var_filter=var_filter
                    )
            if idx == 0:  # only do precip regridding on a single job array task
                self.regrid_to_schism(
                    file, output_path_transformer=output_path_transformer, var_filter=var_filter
                )
    ...
    def regrid_to_schism(self, input_file: Path, output_path_transformer, var_filter=None):
        ...
        if var_filter is not None:
            ...
            if original_name != nc_var.name and self.root:
                pass
        ...
        if self._schism_regridder is None:
            self._schism_regridder = ESMF.Regrid(
                srcfield=in_field,
                dstfield=out_field,
        ...
        out_field = self._schism_regridder(
            srcfield=in_field, dstfield=out_field
        )  # , zero_region=ESMF.constants.Region.SELECT)
    ...
    def regrid_to_lat_lon(self, input_file: Path, output_path_transformer, var_filter=None):
        ...
        if self.root:
            ...
            in_time_name = {"time", "valid_time"}.intersection(set(input_ds.variables)).pop()
        ...
        for variable in ll_vars:
            ...
            if self.root:
                new_var = output_ds.createVariable(
                    varname=nc_var.name, datatype="f4", dimensions=("time", "lat", "lon")
                )
            ...
            if self._regridder is None:
                ...
                self._regridder(
                    srcfield=in_field, dstfield=out_field, zero_region=ESMF.constants.Region.SELECT
                )
