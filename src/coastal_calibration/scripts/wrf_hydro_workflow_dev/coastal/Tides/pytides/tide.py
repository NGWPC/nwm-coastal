...
class Tide:
    ...
    def __init__(self, constituents=None, amplitudes=None, phases=None, model=None, radians=False):
        """
    ...
    def _prepare(constituents, t0, t=None, radians=True):
        ...
        u = [np.mod(np.array([c.u(a_i) for c in constituents])[:, np.newaxis], 360.0) for a_i in a]
        f = [np.mod(np.array([c.f(a_i) for c in constituents])[:, np.newaxis], 360.0) for a_i in a]
    ...
    def _partition(hours, partition=3600.0):
        ...
        total_partitions = np.ceil(relative[-1] / partition + 10 * np.finfo(float).eps).astype(
            "int"
        )
        return [
            hours[np.floor(np.divide(relative, partition)) == i] for i in range(total_partitions)
        ]
    ...
    def at(self, t):
        ...
        return np.concatenate(
            [Tide._tidal_series(t_i, H, p, speed, u_i, f_i, V0) for t_i, u_i, f_i in izip(t, u, f)]
        )
    ...
    def highs(self, *args):
        ...
        see Tide.extrema().
        """
        yield from ifilter(lambda e: e[2] == "H", self.extrema(*args))
    ...
    def lows(self, *args):
        ...
        see Tide.extrema().
        """
        yield from ifilter(lambda e: e[2] == "L", self.extrema(*args))
    ...
    def extrema(self, t0, t1=None, partition=2400.0):
        ...
        if t1:
            ...
            for start, end in izip(*partitions):
                ...
                def d(t):
                    return np.sum(
                        -speed * amplitude * f * np.sin(speed * t + (V0 + u) - phase), axis=0
                    )

                def d2(t):
                    return np.sum(
                        -(speed**2.0) * amplitude * f * np.cos(speed * t + (V0 + u) - phase), axis=0
                    )
    ...
    def decompose(
        ...
        constituents = [c for c in constituents if c != constituent._Z0]
        z0 = np.mean(heights)
        heights = heights - z0
        ...
        def D_residual(hp):
            ...
            ds_dH = np.concatenate(
                [f_i * np.cos(speed * t_i + u_i + V0 - p) for t_i, u_i, f_i in izip(t, u, f)],
                axis=1,
            )

            ds_dp = np.concatenate(
                [H * f_i * np.sin(speed * t_i + u_i + V0 - p) for t_i, u_i, f_i in izip(t, u, f)],
                axis=1,
            )
